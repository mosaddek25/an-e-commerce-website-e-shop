...
import React, { useState, useEffect } from 'react';
import { getCartItems, getProfile, initPayment } from '../../api/apiOrder';  // Make sure initPayment API is defined in your backend API calls
import { userInfo } from '../../utils/auth';
import Layout from '../Layout';
import { Link } from 'react-router-dom';
...
const Checkout = () => {
    ...
    const handlePayment = () => {
        setLoading(true);
        console.log("Payment button clicked.");  // Log to check button press
    
        initPayment(userInfo().token)
            .then(response => {
                console.log("Payment response: ", response);  // Log to check API response
                if (response.data.status === 'SUCCESS') {
                    window.location.replace = response.data.GatewayPageURL;  // Redirect user to payment gateway
                } else {
            ...
            .catch(error => {
                console.error("Error initiating payment: ", error);  // Log to check errors
                setLoading(false);
            });
    }
            ...
            <div className="container">
                <div className="row">
                    ...
                    <div className="col-md-4">
                                ...
                                <ul className="list-group list-group-flush">
                                    {orderItems.map(item => (
                                        <li key={item._id} className="list-group-item" align="right">
                                            {item.product ? item.product.name : ""} x {item.count} = ৳ {item.price * item.count}
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        ...
                        <br />
                        <p>
                            <button className="btn btn-warning btn-md" onClick={handlePayment} disabled={loading}>
